...
The development version of the package is also available from Github.

```{r, eval = FALSE}
BiocManager::install("MarioniLab/MouseGastrulationData")
```

Next, load the package in the typical way.
...
We load in only 20% of the cells in the full dataset to reduce the resources required to compile this vignette.

```{r, message=FALSE}
set.seed(42)
sce <- EmbryoAtlasData(subsample.frac = 0.2)
sce
```
...
After running `r Biocpkg("scater")`'s `normalize` function on the `r Biocpkg("SingleCellExperiment")` object, normalised counts can be accessed using `normcounts`.
Similarly, log-transformed normalised counts may be accessed using `logcounts`
...
A vector of celltype colours (as used in the paper) is also provided, and is accessible via `data(embryo_celltype_colours)`.
Its use is shown below.

```{r, fig.height = 6}
data(embryo_celltype_colours)
head(embryo_celltype_colours, n=3)
...
plot(
    ...
    col = embryo_celltype_colours[colData(sce)$celltype[singlets]],
    pch = 19,
    xaxt = "n", yaxt = "n",
...
```{r}
unfilt <- EmbryoAtlasData(type="raw", raw.samples=c(12, 14))
sapply(unfilt, dim)
```
